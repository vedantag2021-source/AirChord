...
DEFAULT_AUDIO_FOLDER = resource_path("assets/audio")
USER_AUDIO_FOLDER = user_data_path("audio")
...
class GestureEngine:
    ...
    def _load_chord_sounds(self):
        """
        For each chord, prefer a user-regenerated .wav file (saved in
        the writable user data folder) if one exists; otherwise fall
        back to the bundled default that ships with the app.
        """
        sounds = {}
        for chord_name in set(GESTURE_TO_CHORD.values()):
            user_file = os.path.join(USER_AUDIO_FOLDER, f"{chord_name}.wav")
            default_file = os.path.join(DEFAULT_AUDIO_FOLDER, f"{chord_name}.wav")
            file_path = user_file if os.path.isfile(user_file) else default_file
